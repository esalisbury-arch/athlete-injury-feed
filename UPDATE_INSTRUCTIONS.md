...
**Daily target: ~20 new injury records per run.** To hit that volume, don't run a handful of generic searches — run separate, targeted searches per sport/league (e.g. "NFL injury news", "WNBA injury news", "Premier League injury news", "cricket injury news", "NCAA D1 injury news", etc.) so smaller-coverage sports aren't crowded out by NFL/NBA results. If a thorough sweep across all listed sports still turns up fewer than 20 genuinely new high-profile injuries, report the true (lower) count rather than padding it — never fabricate a record to hit the target.

On each run:

1. Read `injuries.json` to see which injuries are already tracked (check the `guid` field to avoid duplicates).
2. Search the web for new high-profile athlete injury news from roughly the last 24-48 hours, across major sports: NFL, NBA, WNBA, MLB, NHL, top soccer leagues (Premier League, La Liga, MLS, NWSL, Champions League, national teams), tennis (ATP/WTA), golf (PGA/LPGA), F1, boxing/UFC, cricket, Olympic sports, and NCAA Division I college sports (football, men's and women's basketball, baseball, softball, volleyball, gymnastics, soccer, wrestling, track & field). Search each sport/league individually rather than combining them into one or two broad queries — this is what surfaces enough volume to reach the ~20/day target. Actively search for women's sports and NCAA D1 college athletes each run, not just the major men's pro leagues — those tend to dominate search results by default.
